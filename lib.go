...
func smuxServerWrapper(kconn *kcp.UDPSession) *smuxServerSideListener {
	listener, err := smux.Server(kconn, nil)
	if err != nil {
...
func smuxClientWrapper(kconn *kcp.UDPSession) *smuxClientSideSession {
	session, err := smux.Client(kconn, nil)
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
	}
